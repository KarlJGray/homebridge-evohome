                ...
                session
                  ...
                  .then(
                    function (thermostats) {
                      session
                        .getSystemModeStatus(locations[that.locationIndex].locationID)
                        .then(
                          function (systemModeStatus) {
                            ...
                            for (var deviceId in locations[that.locationIndex].devices) {
                              for (var thermoId in thermostats) {
                                if (
                                  locations[that.locationIndex].devices[deviceId].zoneID ==
                                  thermostats[thermoId].zoneId
                                ) {
                  ...
                  .then(
                    function (systemModeStatus) {
                      ...
                      for (var deviceId in locations[this.locationIndex].devices) {
                        for (var thermoId in thermostats) {
                          if (
                            locations[this.locationIndex].devices[deviceId].zoneID ==
                            thermostats[thermoId].zoneId
                          ) {
                            for (var i = 0; i < this.myAccessories.length; ++i) {
                              if (
                                ...
                                if (device) {
                                  ...
                                  var oldCurrentTemp = this.myAccessories[i].thermostat
                                    .temperatureStatus.temperature;
                                  var newCurrentTemp = thermostat.temperatureStatus.temperature;
                                  var oldTargetTemp = this.myAccessories[i].thermostat
                                ...
                                !updatedEcoActive &&
                                this.myAccessories[i].systemMode == "AutoWithEco"
                              ) {
...
EvohomeThermostatAccessory.prototype = {
  ...
  getProgramData: function (callback) {
    ...
    var buffer = new Buffer(("" + data).replace(/[^0-9A-F]/gi, ""), "hex").toString("base64");
