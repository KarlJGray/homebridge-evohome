/* 
  THIS IS WORK IN PROGRESS! BE CAREFUL!
  
  What has been changed since the latest version?
  - changed constructor functions to modern classes
  - transfered all "request" to "axios" and got rid of "lodash" and "Q" on the way
  - changed functions to arrow-functions
  - renamed some variables so they are easyer to read and understand
  - moved some URLs to the top
...
class Session {
  ...
  getLocations() {
    ...
    return this.requestData(url).then((json) => {
  ...
  getThermostats(locationId) {
    ...
    return this.requestData(url).then((json) => {
      return json.gateways[0].temperatureControlSystems[0].zones.map((thermostatData) => {
        if (thermostatData.activeFaults.length > 0) {
          // logging system faults to console
          console.log(thermostatData.name);
          console.log(thermostatData.activeFaults);
        }
        /* faults that are possible:
        
        BATTERY LOW
        'activeFaults': [
          {faultType: 'TempZoneSensorLowBattery', since: '2020-05-15T23:07:18'},
          {faultType: 'TempZoneActuatorLowBattery', since: '2020-05-15T23:07:18'}
        ]
        ...
        */
        return new Thermostat(thermostatData);
      });
    });
  }
  ...
  getSchedule(zoneId) {
    ...
    return this.requestData(url).then((json) => {
      return json.dailySchedules.map((scheduleData) => new Schedule(scheduleData));
    });
  }
...
module.exports = {
  login: function (username, password) {
    return login(username, password).then((json) => {
      return getUserInfo(json).then((userinfojson) => {
